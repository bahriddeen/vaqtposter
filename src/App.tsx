  ...
  return <main>
    ...
    <div className="workspace">
      <section className="controls">
        ...
        {config.mode==='namoz'?<>
          <div className="card"><h2><span>01</span> Masjid</h2><label>Masjid nomi<input value={config.mosqueName} onChange={e=>update('mosqueName',e.target.value)} maxLength={45}/></label><small>{config.mosqueName.length}/45 belgi</small><label>Telegram username<input value={config.telegram} onChange={e=>update('telegram',e.target.value)} maxLength={32} placeholder="@username"/></label><small>{config.telegram.length}/32 belgi</small></div>
          ...
          <div className="card"><h2><span>02</span> Muallif</h2><label>Muallif ismi<input value={config.author} onChange={e=>update('author',e.target.value)} maxLength={50} placeholder="Kim aytgan?"/></label><small>{config.author.length}/50 belgi</small><label>Kitob, sana yoki izoh<input value={config.additionalInfo} onChange={e=>update('additionalInfo',e.target.value)} maxLength={80} placeholder="Kitob nomi, sana..."/></label><small>{config.additionalInfo.length}/80 belgi</small><label>Telegram username<input value={config.telegram} onChange={e=>update('telegram',e.target.value)} maxLength={32} placeholder="@username"/></label><small>{config.telegram.length}/32 belgi</small></div>
        </>}
        <div className="card"><h2><span>03</span> Fon</h2><div className="backgrounds">{backgrounds.map(bg=><button key={bg.id} className={'bg-choice '+(config.background.id===bg.id?'active':'')} onClick={()=>update('background',bg)}><svg viewBox="0 0 1080 1440" preserveAspectRatio="xMidYMid slice">{bg.source?<image href={bg.source} width="1080" height="1440"/>:<BackgroundArt kind={bg.kind} light={bg.light}/>}</svg><span>{bg.name}</span></button>)}<label className="upload"><Upload size={20}/><span>Rasm yuklash</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>upload(e.target.files?.[0])}/></label></div>
